...
class EnergyPlusModel(metaclass=ABCMeta):
    ...
    def set_action(self, normalized_action, framework):
        ...
        if framework == "openai":
            self.action = self.action_space_limits[:,0] + (normalized_action + 1.) * 0.5 * (self.action_space_limits[:,1] - self.action_space_limits[:,0])
            self.action = np.clip(self.action, self.action_space_limits[:,0], self.action_space_limits[:,1])
        else:
    ...
    def plot(self, log_dir='', csv_file='', **kwargs):
        """
        Plot method for visualizing EnergyPlus simulation results.

        Parameters:
        - log_dir (str): Directory path to save the plot if provided.
        - csv_file (str): Path to the CSV file containing simulation results.
        - **kwargs: Additional keyword arguments.

        If log_dir is specified, the method assumes it's a directory containing simulation logs,
        and it shows the progress of the simulation.

        If csv_file is specified, the method assumes it's a CSV file containing simulation episode results.
        It reads the episode data, configures plot settings, and plots the simulation results.

        If neither log_dir nor csv_file is provided or if the specified paths are invalid, an error message is printed.

        Note: The plot is saved as a PNG file in the log directory if log_dir is specified.

        Example Usage:
        energyplus_model = EnergyPlusModel()
        energyplus_model.plot(log_dir='logs/', csv_file='results.csv')
        """    
        if log_dir != '':
            # Check if log_dir is a valid directory
            if not os.path.isdir(log_dir):
                print('energyplus_model.plot: {} is not a directory'.format(log_dir))
                return
            
            # Set log_dir and show the progress of the simulation
            print('energyplus_plot.plot log={}'.format(log_dir))
            self.log_dir = log_dir
            self.show_progress()
        else:
            # Check if csv_file is a valid file
            if not os.path.isfile(csv_file):
                print('energyplus_model.plot: {} is not a file'.format(csv_file))
                return
            print('energyplus_model.plot csv={}'.format(csv_file))
            
            # Read episode data from the specified CSV file
            ...
            # plt.show()
            # Save the lot as a PNG file in the log directort
            fig_location = log_dir + '/testFigure.png'
    ...
    def show_progress(self):
        ...
        # Plot second to last episode, as last episode contains errors due to early stopping
        self.update_episode(self.num_episodes - 1)

        # Save the plot
        fig_location = self.log_dir + '/testFigure.png'
        plt.savefig(fig_location)
