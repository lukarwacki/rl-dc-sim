...
def train_agent(env, args):
    """
    Train the A2C agent on the provided environment.
...
def main():
    """
    This function handles the training of the A2C agent on the EnergyPlus environment. 
    ...
    # Modify settings
    args.num_episodes = 25                           # Number of episodes to train the agent
    ...
    args.learning_rate = 0.013786395748848884      # Learning rate of the NNs in the A2C
    args.learning_rate_schedule = 'linear'          # Learning rate schedule over the training iterations
    args.n_steps = 784                              # Number of steps to collect samples for each training iteration
    args.gamma = 0.8298643947670976                 # Discount factor
    args.GAE_lambda = 0.8491212786622184            # Factor for the Generalized Advantage Estimation
    args.clip_range = 0.4487117411242193            # Clipping range for the A3C
    args.clip_range_schedule = 'linear'             # Clipping range schedule over the training iterations
    args.policy_kwargs = dict(net_arch=dict(pi=[28],vf=[18]))   # Architecture of the NNs in the A3C
    args.ent_coef = 0.00020139350791915665            # Entropy coefficient for the A3C
    args.vf_coef = 0.7709706190672836               # Value function coefficient for the A3C
